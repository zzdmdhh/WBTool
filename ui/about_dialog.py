...
class AboutDialog(QDialog):
    ...
    def _build_banner(self):
        ...
        if image is not None:
            ...
            logo.setStyleSheet(
                ...
                f"color: {common.COLOR_PRIMARY}; font-size: 30px; font-weight: bold;"
            )

        # 名称 + 简介
        text_box = QVBoxLayout()
        text_box.setSpacing(5)
        name = QLabel(common.APP_NAME)
        name.setStyleSheet("color: #FFFFFF; font-size: 26px; font-weight: bold;")
        desc = QLabel(common.APP_DESC)
        ...
        # 版本徽标
        badge = QLabel(f"{common.APP_STAGE} {common.APP_VERSION}")
    ...
    def _build_body(self):
        ...
        license_title.setStyleSheet(
            f"color: {common.COLOR_TEXT_DARK}; font-size: 15px; font-weight: bold;"
        )
        license_text = QLabel(
        ...
        license_text.setStyleSheet(
            f"color: {common.COLOR_TEXT_DARK}; font-size: 13px;"
        )

        # 开源仓库链接（地址待补充时显示占位文字）
        if common.APP_REPO_URL:
            repo = QLabel(
                f'<a href="{common.APP_REPO_URL}" '
                f'style="color:{common.COLOR_PRIMARY}; text-decoration:none;">'
                "开源仓库地址 →</a>"
            )
    ...
    def _style(self):
        ...
        QFrame#banner {{
            background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
                stop:0 {common.COLOR_PRIMARY}, stop:1 #5B93FF);
        ...
        QPushButton#primary {{
            background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                stop:0 {common.COLOR_PRIMARY}, stop:1 #4C8DFF);
            border: none;
            color: #FFFFFF;
            font-weight: bold;
        }}
        ...
        QPushButton#primary:pressed {{
            background: {common.COLOR_PRIMARY_DARK};
        }}
        """
    ...
    def _load_logo(self, size):
        ...
        if not os.path.exists(common.LOGO_PATH):
            return None
        pixmap = QPixmap(common.LOGO_PATH)
        if pixmap.isNull():
            return None
    ...
    def check_update(self):
        ...
        if not common.UPDATE_URL:
            QMessageBox.information(
                self, "检查更新",
                "更新服务器地址尚未配置。\n"
                "请在 utils/common.py 中填写 UPDATE_URL 后重试。",
            )
            return
        ...
        request = QNetworkRequest(QUrl(common.UPDATE_URL))
    ...
    def _on_update_finished(self):
        ...
        if self._compare_version(latest, common.APP_VERSION) > 0:
            message = f"发现新版本：{common.APP_STAGE} {latest}"
            ...
            QMessageBox.information(
                self, "检查更新",
                f"当前已是最新版本（{common.APP_STAGE} {common.APP_VERSION}）。",
            )
