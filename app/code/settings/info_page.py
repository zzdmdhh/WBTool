...
"""
信息面板设置页面
配置信息面板的刷新间隔、面板宽度，并提供配置文件（info.json）的快速打开入口。
"""
...
class InfoPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self.width_spin.setFixedWidth(140)
        layout.addLayout(make_form_row("面板宽度", self.width_spin))

        # 配置文件
        self.path_label = QLabel(INFO_JSON_PATH)
        ...
        hint = QLabel("信息条目（标签、内容、动态占位符）在配置文件中编辑，保存后自动生效")
    ...
    def load_values(self, values):
        ...
        self.width_spin.setValue(int(values.get("panel_width", 230)))

    def values(self):
        """从界面控件收集设置值。"""
        return {
            "refresh_seconds": self.refresh_spin.value(),
            "panel_width": self.width_spin.value(),
        }
    ...
    def _open_config(self):
        """使用系统默认编辑器打开信息面板配置文件。"""
        try:
            os.startfile(INFO_JSON_PATH)  # Windows 专用
        except OSError:
